...
class SetChannelCommand extends Command {
  constructor(client) {
    super(client, {
      name: 'setchannel',
      aliases: ['channel'],
      description: 'Configura el canal en el que el bot anunciará las ofertas de juegos gratis.',
      emoji: ':loudspeaker:',
      group: 'config',
      guildOnly: true,
      ownerOverride: false,
  ...
  async updateChannel(message, channel) {
    ...
    if (previousChannelID === channel.id) {
      return message.reply('el canal que mencionaste ya está configurado como canal de anuncios.');
    }
    ...
    try {
      ...
      return message.channel.send(`Canal de anuncios establecido en ${channel}.`);
    } catch (error) {
      logger.error(error);
      return message.channel.send('Algo sucedió al intentar actualizar el canal de anuncios.');
    }
  }
  ...
  run(message) {
    ...
    if (!channel.viewable) {
      return message.reply(`No puedo ver el canal que mencionaste, ¿tengo suficientes permisos para acceder a él?`);
    }
